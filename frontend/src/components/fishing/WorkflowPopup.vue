  ...
  <main>
    ...
    <div class="container">
      ...
      <div class="content" v-else>
        <div class="req-title">요청 구역</div>
        <div class="immediately-props">
          <div class="d-flex a-i-center j-c-between">
            <button class="draw-btn" @click="toggleDraw('req:draw')">
              {{ props.selectedCoords ? '구역 재설정' : '지도에서 구역 설정' }}
            </button>
          </div>
          <div v-if="props.selectedCoords" class="coords-log">
            <small>설정된 좌표 ({{ props.selectedCoords[0].length }} points)</small>
