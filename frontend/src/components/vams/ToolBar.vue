<template>
  <section id="toolbar" class="d-flex">
    <div class="d-flex tools">
      ...
      <div class="districtmap-config d-flex">
        <button @click="emit('imglist:toggle')">위성영상 리스트</button>
      </div>
      <div class="divider"></div>
      ...
      <div class="divider"></div>
      <div class="section-list d-flex">
        <button style="width: 120px;" @click="displayRequest = !displayRequest" :class="[displayRequest ? 'req-activate' : '']">워크플로우 등록</button>
      </div>
    </div>
...
const emit = defineEmits(['section:change', 'trenchmap:change', 'filter:change', 'districtmapconfig:toggle', 'imglist:toggle', 'toolbar:draw'])
